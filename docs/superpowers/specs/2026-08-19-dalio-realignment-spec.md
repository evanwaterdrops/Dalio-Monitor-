...
| sofr, iorb | FRED `SOFR`,`IORB` | fast/trigger | print discriminator: SOFR−IORB ≥ +10bp persistent = funding stress; WALCL expansion without stress & bills-led = reserve management, NOT monetization | Fed abandons ample-reserves regime |
| spx | Stooq `^spx` (Yahoo fallback) | fast/trigger | drawdown ruler: −20% elevated, −40% critical vs 3y high (Dalio ~50% depression P1:1085); Shiller real total-return monthly for backtest | — |
